---
target_file: "bones/scripts/rc-apex-adapter.sh"
date: "2026-08-12T00:38:36Z"
template: "rotkeeper-doc.html"
status: "stub"
version: "0.1.0"
author: "Rotkeeper DIP"
project: "Rotkeeper"
---

# rc-apex-adapter

Documentation for `bones/scripts/rc-apex-adapter.sh`. This file was auto-generated by the Document Improvement Project (DIP).

## Overview
<!-- DIP-GENERATED-MARKER: Overview -->
TODO: Provide a brief overview of what this file does.

###### CLI Usage
<!-- DIP-HELP-EXTRACTED: 2026-08-12T12:26:46Z -->

*Not found: no help block for `rc-apex-adapter.sh` in autopsy help report.*
## Environment
<!-- DIP-ENV-EXTRACTED: 2026-08-12T00:38:36Z -->

- **$ROOT_DIR**: .
- **$OUTPUT_DIR**: output
- **$CONTENT_DIR**: home/content
- **$ASSETS_DIR**: home/assets
- **$DOCS_DIR**: home/content/docs
- **$HELP_DIR**: home/content/help
- **$BONES_DIR**: bones
- **$SCRIPT_DIR**: bones/scripts
- **$CONFIG_DIR**: bones/config
- **$LOG_DIR**: bones/logs
- **$TMP_DIR**: bones/tmp
- **$ARCHIVE_DIR**: bones/archive
- **$REPORT_DIR**: bones/reports
- **$BOOK_REPORT_DIR**: bones/book-reports
- **$TEMPLATE_DIR**: bones/templates
- **$META_DIR**: bones/meta
- **$WEB_DIR**: output
## Ritual History
<!-- DIP-HISTORY-EXTRACTED: 2026-08-12T00:38:36Z -->

*Not found: no changelog/history entries matching `rc-apex-adapter.sh`.*
## Necromancer's Notes
<!-- DIP-SOUL-EXTRACTED: 2026-08-12T00:38:36Z -->

*Not found: no soul sidecar for `bones/scripts/rc-apex-adapter.sh`.*
