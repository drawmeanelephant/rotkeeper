...
│   ├── archives/                # Release distributions and ingested archive vault
...
> **Note:** `bones/archive/` (singular) is used by `pack` for tomb archives. `bones/archives/` (plural) is used by `release` for distribution zips and by `ingest` for processed archives. Both directories exist and serve different purposes.
...
| `release` | Create versioned `lite` and `full` distribution `.zip` files in `bones/archives/releases/`. |
| `ingest` | Unpack `.tar.gz` payloads from `messages-from-my-friends/` into `home/content/messages/`. Moves processed archives to `bones/archives/ingested/`. |
...
5. **The central repository** runs `./rotkeeper.sh ingest` to:
   ...
   - Move processed archives to `bones/archives/ingested/`
...
|-------------|----------------|-----------------|
| **`full`** | Everything except `.git/`, `tmp/`, `output/`, `bones/logs/`, `bones/archives/` | Nothing significant |
...
- **Two archive directories exist:** `bones/archive/` (pack output) and `bones/archives/` (releases, ingested). This is intentional.
