...
if [[ "${1:-}" == "wrap" ]]; then
  ...
  # v3 generic hook (#269): any meta-json key beyond the typed set interpolates.
  # meta_has distinguishes present (substitute, null→empty) from absent (verbatim).
  function meta_has(meta, tok,   arr) { return (match(meta, "\"" tok "\"[[:space:]]*:", arr) > 0) ? 1 : 0 }
  function meta_lookup(meta, tok,   arr, arr2, arr3) { if (match(meta, "\"" tok "\"[[:space:]]*:[[:space:]]*\"([^\"]*)\"", arr)) return arr[1]; if (match(meta, "\"" tok "\"[[:space:]]*:[[:space:]]*([0-9.-]+)", arr2)) return arr2[1]; if (match(meta, "\"" tok "\"[[:space:]]*:[[:space:]]*(true|false|null)", arr3)) { if (arr3[1] == "null") return ""; return arr3[1] } return "" }
  function generic_if(tmpl, meta,   out, rest, sp, ep, e, name, val, inner) { rest=tmpl; out=""; while((sp=index(rest,"$if("))>0){ out=out substr(rest,1,sp-1); rest=substr(rest,sp+4); ep=index(rest,")$"); if(ep==0){ out=out "$if(" rest; rest=""; break } name=substr(rest,1,ep-1); rest=substr(rest,ep+2); e=index(rest,"$endif$"); if(e==0){ out=out "$if(" name ")$" rest; rest=""; break } if (!meta_has(meta,name)) { out=out "$if(" name ")$" substr(rest,1,e-1) "$endif$"; rest=substr(rest,e+7) } else { val=meta_lookup(meta,name); if(val==""){ rest=substr(rest,e+7) } else { inner=substr(rest,1,e-1); out=out inner; rest=substr(rest,e+7) } } } return out rest }
  function generic_subst(tmpl, meta,   out, rest, pos, ce, tok, val) { out=""; rest=tmpl; while((pos=index(rest,"$"))>0){ out=out substr(rest,1,pos-1); rest=substr(rest,pos+1); ce=index(rest,"$"); if(ce==0){ out=out "$" rest; rest=""; break } tok=substr(rest,1,ce-1); rest=substr(rest,ce+1); if (!meta_has(meta,tok)) { out=out "$" tok "$" } else { val=meta_lookup(meta,tok); out=out html_escape(val) } } return out rest }
  BEGIN { title_esc=html_escape(title); desc_esc=html_escape(desc); author_esc=html_escape(author); date_esc=html_escape(date); palette_esc=html_escape(palette); version_esc=html_escape(version); subtitle_esc=html_escape(subtitle); tags_esc=html_escape(tags); asset_meta_esc=html_escape(asset_meta); body=""; while((getline line < body_file)>0){body=body line "\n"} close(body_file); if(length(body)>0 && substr(body,length(body))=="\n") body=substr(body,1,length(body)-1); tmpl=""; while((getline line < template_file)>0){tmpl=tmpl line "\n"} close(template_file); tmpl=evaluate_if(tmpl,"title",title); tmpl=evaluate_if(tmpl,"description",desc); tmpl=evaluate_if(tmpl,"author",author); tmpl=evaluate_if(tmpl,"date",date); tmpl=evaluate_if(tmpl,"palette",palette); tmpl=evaluate_if(tmpl,"version",version); tmpl=evaluate_if(tmpl,"subtitle",subtitle); tmpl=evaluate_if(tmpl,"tags",tags); tmpl=evaluate_if(tmpl,"asset_meta",asset_meta); tmpl=literal_replace(tmpl,"$title$",title_esc); tmpl=literal_replace(tmpl,"$description$",desc_esc); tmpl=literal_replace(tmpl,"$author$",author_esc); tmpl=literal_replace(tmpl,"$date$",date_esc); tmpl=literal_replace(tmpl,"$palette$",palette_esc); tmpl=literal_replace(tmpl,"$version$",version_esc); tmpl=literal_replace(tmpl,"$subtitle$",subtitle_esc); tmpl=literal_replace(tmpl,"$tags$",tags_esc); tmpl=literal_replace(tmpl,"$asset_meta$",asset_meta_esc); tmpl=literal_replace(tmpl,"$assets_root$",ar); tmpl=literal_replace(tmpl,"$body$",body); meta=""; while((getline line < meta_file)>0){meta=meta line} close(meta_file); tmpl=generic_if(tmpl,meta); tmpl=generic_subst(tmpl,meta); printf "%s",tmpl; exit }'
  exit 0
fi
...
if [[ "${1:-}" == "wrap" ]]; then
  ...
  # html_escape escapes &<>"' for meta; literal_replace does index() literal substitution; evaluate_if handles $if(var)$...$endif$
  gawk -v title="$title" -v desc="$desc" -v author="$author" -v date="$date" -v palette="$palette" -v version="$version" -v subtitle="$subtitle" -v tags="$tags" -v asset_meta="$asset_meta" -v assets_root="$ar" -v body_file="$bf" -v template_file="$tpl" -v meta_file="$mj" 'function html_escape(str,   s) { s=str; gsub(/&/,"\\&amp;",s); gsub(/</,"\\&lt;",s); gsub(/>/,"\\&gt;",s); gsub(/"/,"\\&quot;",s); gsub(/\x27/,"\\&#39;",s); return s } function literal_replace(str, search, replace,   pos, len, result, tail) { len=length(search); result=""; tail=str; while((pos=index(tail,search))>0){result=result substr(tail,1,pos-1) replace; tail=substr(tail,pos+len)} return result tail } function evaluate_if(tmpl, var_name, var_val,   start_tag, end_tag, sp, ep, before, after, inner) { start_tag="$if(" var_name ")$"; end_tag="$endif$"; while((sp=index(tmpl,start_tag))>0){ep=index(substr(tmpl,sp),end_tag); if(ep==0) break; ep=sp+ep-1+length(end_tag)-1; before=substr(tmpl,1,sp-1); after=substr(tmpl,ep+1); if(var_val==""||var_val=="null"){tmpl=before after}else{inner=substr(tmpl,sp+length(start_tag),ep-sp-length(start_tag)-length(end_tag)+1); tmpl=before inner after} } return tmpl } function meta_has(meta, tok,   arr) { return (match(meta, "\"" tok "\"[[:space:]]*:", arr) > 0) ? 1 : 0 } function meta_lookup(meta, tok,   arr, arr2, arr3) { if (match(meta, "\"" tok "\"[[:space:]]*:[[:space:]]*\"([^\"]*)\"", arr)) return arr[1]; if (match(meta, "\"" tok "\"[[:space:]]*:[[:space:]]*([0-9.-]+)", arr2)) return arr2[1]; if (match(meta, "\"" tok "\"[[:space:]]*:[[:space:]]*(true|false|null)", arr3)) { if (arr3[1] == "null") return ""; return arr3[1] } return "" } function generic_if(tmpl, meta,   out, rest, sp, ep, e, name, val, inner) { rest=tmpl; out=""; while((sp=index(rest,"$if("))>0){ out=out substr(rest,1,sp-1); rest=substr(rest,sp+4); ep=index(rest,")$"); if(ep==0){ out=out "$if(" rest; rest=""; break } name=substr(rest,1,ep-1); rest=substr(rest,ep+2); e=index(rest,"$endif$"); if(e==0){ out=out "$if(" name ")$" rest; rest=""; break } if (!meta_has(meta,name)) { out=out "$if(" name ")$" substr(rest,1,e-1) "$endif$"; rest=substr(rest,e+7) } else { val=meta_lookup(meta,name); if(val==""){ rest=substr(rest,e+7) } else { inner=substr(rest,1,e-1); out=out inner; rest=substr(rest,e+7) } } } return out rest } function generic_subst(tmpl, meta,   out, rest, pos, ce, tok, val) { out=""; rest=tmpl; while((pos=index(rest,"$"))>0){ out=out substr(rest,1,pos-1); rest=substr(rest,pos+1); ce=index(rest,"$"); if(ce==0){ out=out "$" rest; rest=""; break } tok=substr(rest,1,ce-1); rest=substr(rest,ce+1); if (!meta_has(meta,tok)) { out=out "$" tok "$" } else { val=meta_lookup(meta,tok); out=out html_escape(val) } } return out rest } BEGIN { title_esc=html_escape(title); desc_esc=html_escape(desc); author_esc=html_escape(author); date_esc=html_escape(date); palette_esc=html_escape(palette); version_esc=html_escape(version); subtitle_esc=html_escape(subtitle); tags_esc=html_escape(tags); asset_meta_esc=html_escape(asset_meta); body=""; while((getline line < body_file)>0){body=body line "\n"} close(body_file); if(length(body)>0 && substr(body,length(body))=="\n") body=substr(body,1,length(body)-1); tmpl=""; while((getline line < template_file)>0){tmpl=tmpl line "\n"} close(template_file); tmpl=evaluate_if(tmpl,"title",title); tmpl=evaluate_if(tmpl,"description",desc); tmpl=evaluate_if(tmpl,"author",author); tmpl=evaluate_if(tmpl,"date",date); tmpl=evaluate_if(tmpl,"palette",palette); tmpl=evaluate_if(tmpl,"version",version); tmpl=evaluate_if(tmpl,"subtitle",subtitle); tmpl=evaluate_if(tmpl,"tags",tags); tmpl=evaluate_if(tmpl,"asset_meta",asset_meta); tmpl=literal_replace(tmpl,"$title$",title_esc); tmpl=literal_replace(tmpl,"$description$",desc_esc); tmpl=literal_replace(tmpl,"$author$",author_esc); tmpl=literal_replace(tmpl,"$date$",date_esc); tmpl=literal_replace(tmpl,"$palette$",palette_esc); tmpl=literal_replace(tmpl,"$version$",version_esc); tmpl=literal_replace(tmpl,"$subtitle$",subtitle_esc); tmpl=literal_replace(tmpl,"$tags$",tags_esc); tmpl=literal_replace(tmpl,"$asset_meta$",asset_meta_esc); tmpl=literal_replace(tmpl,"$assets_root$",ar); tmpl=literal_replace(tmpl,"$body$",body); meta=""; while((getline line < meta_file)>0){meta=meta line} close(meta_file); tmpl=generic_if(tmpl,meta); tmpl=generic_subst(tmpl,meta); printf "%s",tmpl; exit }'
  exit 0
fi
...
S2V2_EMPTY_EOF
    ...
    # --- S2-v3: Generic frontmatter hook (#269) ---
    # The adapter merges every scalar frontmatter key into wrap_meta; wrap
    # interpolates any present key (v3 generic hook). The fake mirrors the
    # dialect: present → substitute (null → empty), absent → verbatim.
    echo "  [+] Executing S2-v3 generic frontmatter hook assertions ($mode)..."
    cat << 'S2V3_EOF' > "$b_templates/custom-s2-v3.html"
<div data-page-type="$page_type$">
<p class="cnt">$count$</p>
<p class="g">$if(page_type)$GATED:$page_type$$endif$</p>
<p class="miss">$missing$</p>
<p class="ifmiss">$if(missing)$X$endif$</p>
</div>
S2V3_EOF
    cat << 'S2V3_FM_EOF' > "$b_content/template-s2-v3.md"
---
title: "S2V3"
page_type: "404"
slug: "/tomb"
count: 7
template: "custom-s2-v3.html"
---
S2V3 body.
S2V3_FM_EOF
    RK_OLIVER_BIN="$fake_bin" ./rotkeeper.sh render > /dev/null
    rendered_s2v3="$out_dir_rel/template-s2-v3.html"
    if [[ ! -f "$rendered_s2v3" ]]; then
      echo "❌ Assertion Failed: template-s2-v3.html missing (S2-v3)."
      exit 250
    fi
    if ! grep -q 'data-page-type="404"' "$rendered_s2v3"; then
      echo "❌ Assertion Failed: S2-v3 generic \$page_type\$ substitution failed."
      exit 251
    fi
    if ! grep -q '<p class="cnt">7</p>' "$rendered_s2v3"; then
      echo "❌ Assertion Failed: S2-v3 non-string scalar (\$count\$) failed."
      exit 252
    fi
    if ! grep -q '<p class="g">GATED:404</p>' "$rendered_s2v3"; then
      echo "❌ Assertion Failed: S2-v3 generic \$if\$ gate failed."
      exit 253
    fi
    # shellcheck disable=SC2016
    if ! grep -q '<p class="miss">$missing$</p>' "$rendered_s2v3"; then
      echo "❌ Assertion Failed: S2-v3 absent token should pass verbatim."
      exit 254
    fi
    # shellcheck disable=SC2016
    if ! grep -q '<p class="ifmiss">$if(missing)$X$endif$</p>' "$rendered_s2v3"; then
      echo "❌ Assertion Failed: S2-v3 absent \$if\$ should pass verbatim."
      exit 255
    fi
    # Real Oliver wrap v3 probe: any meta-json key interpolates.
    _real_wrap3="${RK_OLIVER_BIN:-}"
    if [[ -z "$_real_wrap3" || ! -x "$_real_wrap3" ]]; then
      _real_wrap3="$(command -v oliver 2>/dev/null || true)"
    fi
    if [[ -n "$_real_wrap3" && -x "$_real_wrap3" ]]; then
      tmp_meta_wrap_v3="$pass_dir/bones/tmp/meta-wrap-v3-probe.json"
      printf '{"title":"","description":"","author":"","date":"","palette":"","version":"%s","subtitle":"","tags":"","asset_meta":"","page_type":"404","count":7}' "$exp_ver" > "$tmp_meta_wrap_v3"
      if ! "$_real_wrap3" wrap --template "$b_templates/custom-s2-v3.html" --meta-json "$tmp_meta_wrap_v3" --assets-root "./assets/" --body "$tmp_body" | grep -q 'data-page-type="404"'; then
        echo "❌ Assertion Failed: real Oliver wrap v3 generic interpolation failed."
        exit 225
      fi
      echo "  [+] Pass: real Oliver wrap v3 generic token interpolation ($mode)."
    fi
    echo "  [+] Pass: S2-v3 generic frontmatter hook assertions ($mode)."
